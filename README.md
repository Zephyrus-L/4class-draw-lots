...
界面采用 Apple 风格 Windows 桌面规范：内容优先的侧栏布局、语义化 CSS 令牌、浅色/深色主题、键盘焦点状态和 reduced-motion 支持。此次界面改造仅涉及 React 前端样式，不改变业务数据结构、抽签算法或 Tauri 命令。
...
前端采用 Notion 风格界面：使用米白画布、克制灰阶、轻量边框和低噪声悬停反馈，保持固定侧栏与主内容独立滚动，并支持浅色/深色系统主题、可见键盘焦点和 reduced-motion。
