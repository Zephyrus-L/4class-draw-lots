...
import { Clipboard, Download, FileUp, Github, History, Info, List, Play, Plus, RotateCcw, Settings2, Trash2, X } from 'lucide-react'
...
function useAppData() {
  ...
  useEffect(() => {
    invoke('load_data').then((saved) => setData(normalize(saved))).catch(() => { try { const saved = localStorage.getItem('draw-lottery-data'); if (saved) setData(normalize(JSON.parse(saved))) } catch {} })
    invoke('load_probabilities').then(setWeights).catch(() => {})
  }, [])
  const save = (next) => { const normalized = normalize(next); setData(normalized); localStorage.setItem('draw-lottery-data', JSON.stringify(normalized)); invoke('save_data', { data: normalized }).catch(() => {}) }
  ...
  const draw = () => {
    ...
    const animate = (delay, index) => {
      rollingTimer.current = window.setTimeout(() => {
        ...
        if (delay < 280) animate(delay + 28, index + 1)
      }, delay)
    }
    animate(55, 0)
    setRolling(true)
  ...
  return <div className="app-shell" onPointerUp={endDrag}>
    <div className="workspace"><aside className="sidebar"><div className="sidebar-brand"><img className="brand-icon" src="/icons/app-icon.png" alt="Foclas抽签" /><div><strong>Foclas抽签</strong><small>简洁 · 专注 · 公平</small></div></div><nav><button disabled={rolling} className={tab === 'draw' ? 'active' : ''} onClick={() => setTab('draw')}><Play size={17} />开始抽签</button><button disabled={rolling} className={tab === 'people' ? 'active' : ''} onClick={() => setTab('people')}><List size={17} />名单管理</button><button disabled={rolling} className={tab === 'history' ? 'active' : ''} onClick={() => setTab('history')}><History size={17} />抽签记录<span className="nav-count">{data.draws.length}</span></button><button disabled={rolling} className={tab === 'about' ? 'active' : ''} onClick={() => setTab('about')}><Info size={17} />关于</button></nav><div className="sidebar-foot"><Settings2 size={15} />数据保存在程序目录 / data</div></aside>
      <main className="main-content">{groupBar}
        ...
        {tab === 'about' && <section className="content-page about-page"><p className="eyebrow">关于应用</p><h1>Foclas抽签</h1><p className="muted">简洁、专注、公平的本地抽签工具。</p><div className="about-block"><Info size={20} /><div><strong>当前版本</strong><p>1.0.0</p></div></div><a className="about-link" href="https://github.com/Zephyrus-L/foclas-draw-lots" target="_blank" rel="noreferrer"><Github size={18} />访问 GitHub 项目</a></section>}
      </main></div></div>
}
